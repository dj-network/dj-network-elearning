...
export default async function TheoriePage({ searchParams }) {
  ...
  const categoryRec = await db
    .select()
    .from(categories)
    .where(eq(categories.slug, "theorie"))
    .limit(1);
  ...
  const formatProducts = dbProducts
    ...
    const tag = tags[0] || "Théorie Musicale";
        ...
        <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <p className="text-slate-400 max-w-2xl">
              Supports de cours sur l&apos;harmonie, le rythme, la structure de
              morceaux. Approfondissez les fondamentaux de la musique.
            </p>
          </div>
        </div>
