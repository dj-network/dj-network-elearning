...
export default async function PacksSamplesPage({ searchParams }) {
  ...
  const categoryRec = await db
    .select()
    .from(categories)
    .where(eq(categories.slug, "packs-samples"))
    .limit(1);
  ...
  // Parse tags string into first tag
        ...
        <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <p className="text-slate-400 max-w-2xl">
              Kicks, snares, boucles mélodiques, atmosphères. Des packs de
              samples professionnels pour tous les genres.
            </p>
          </div>
        </div>
