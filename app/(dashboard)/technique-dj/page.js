...
export default async function TechniqueDJPage({ searchParams }) {
  ...
  const categoryRec = await db
    .select()
    .from(categories)
    .where(eq(categories.slug, "technique-dj"))
    .limit(1);
        ...
        <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <h1 className="text-3xl font-black text-white">Technique DJ</h1>
            </div>
            <p className="text-slate-400 max-w-2xl">
              Mapping de contrôleurs, routines de scratch, préparation de
              playlists. Améliorez votre technique de mix.
            </p>
          </div>
        </div>
